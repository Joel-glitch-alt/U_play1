...
class RewardsController extends GetxController {
  ...
  RewardsController({required this.registereRepo});

  //
  var moneyZoneTypesList = <Map<String, dynamic>>[];
  ...
  @override
  void onInit() {
    const String MONEYZONE = AppConstants.MONEYZONE;
    print("[DEBUG] - Controller : ${this.runtimeType}");
    getMONEYZONE(MONEYZONE);

    super.onInit();
  }

  //
  Future<void> getMONEYZONE(String url) async {
    print(url);
    try {
      ...
      if (response.statusCode == 200) {
        ...
        update();
        print(parsedData);
        moneyZoneTypesList = List<Map<String, dynamic>>.from(parsedData
            .map((region) => {
