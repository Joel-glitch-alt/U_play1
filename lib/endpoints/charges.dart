...
class ChargesController extends GetxController {
  ...
  ChargesController({required this.registereRepo});

  //
  var chargesTypesList = <Map<String, dynamic>>[];
  ...
  @override
  void onInit() {
    const String CHARGES = AppConstants.POINTS;
  ...
  //
  Future<void> getCharges(String url) async {
    print(url);
    try {
      ...
      if (response.statusCode == 200) {
        ...
        update();
        print(parsedData);
        chargesTypesList = List<Map<String, dynamic>>.from(parsedData
            .map((region) => {
