...
class GiftZonesController extends GetxController {
  final RegistereRepo registereRepo;
  GiftZonesController({required this.registereRepo});

  //
  var giftzonesTypesList = <Map<String, dynamic>>[];
  List<dynamic> GIFTZONES = [];
  //
  ...
  @override
  void onInit() {
    const String GIFTSZONE = AppConstants.GIFTSZONE;
    print("[DEBUG] - Controller : ${this.runtimeType}");
    getGiftZones(GIFTSZONE);

    super.onInit();
  }

  //
  Future<void> getGiftZones(String url) async {
    print(url);
    try {
      ...
      if (response.statusCode == 200) {
        ...
        update();
        print(parsedData);
        giftzonesTypesList = List<Map<String, dynamic>>.from(parsedData
            .map((region) => {
